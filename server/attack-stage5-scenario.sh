...
HEADER_FILE="/tmp/attack_stage5_headers.txt"
BODY_FILE="/tmp/attack_stage5_body.txt"
CODE_FILE="/tmp/attack_stage5_code.txt"
...
log "Stage5 취약점 검증 시나리오 시작 (Base: ${BASE_URL})"
...
if [ -z "$REFRESH_COOKIE" ]; then
  ...
  log "8) Stage5: 다중 401 레이스 동시성 제어(동시 refresh 중복 사용)"
  # 동시성 테스트는 별도 세션으로 실행해 이전 테스트의 재사용 영향 배제
  http_call 1 -X POST "${BASE_URL}/login" -H 'Content-Type: application/json' -d '{"username":"demo","password":"demo"}'
  capture_response
  CONCURRENT_REFRESH_COOKIE="$(extract_refresh_cookie "$HEADER_FILE")"
  ...
  if [ -z "$CONCURRENT_REFRESH_COOKIE" ]; then
    ...
    if [ "$ST5_SUCCESS_COUNT" -ge 2 ]; then
      vulnerable "refresh 동시성 제어 미흡" "동일 refresh 동시 호출이 모두 200 (1회만 갱신해야 함): 결과=${ST5_STATUS1},${ST5_STATUS2}"
      ...
      secure "refresh 동시성 제어" "동일 refresh 동시 호출 제어됨(200 한 번)"
    else
...
if [ "$VULNERABLE_COUNT" -gt 0 ]; then
  echo "[SUMMARY] Stage5에서 재현 가능한 취약점이 확인되었습니다."
  exit 2
fi
echo "[SUMMARY] Stage5 기준 즉시 재현 취약점이 없습니다."
